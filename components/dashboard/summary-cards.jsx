...
import { vehicles, maintenanceRecords, logEntries } from "@/lib/data"

const totalVehicles = vehicles.length
const availableVehicles = vehicles.filter((v) => v.status === "available").length
const inUseVehicles = vehicles.filter((v) => v.status === "in-use").length
const maintenanceVehicles = vehicles.filter((v) => v.status === "maintenance").length
const monthlyFuelCost = logEntries.reduce((sum, l) => sum + l.fuelCost, 0)
const pendingMaintenance = maintenanceRecords.length
